...
def convert_currency(amount, from_currency, to_currency):
    ...
    return round(result, 4), (
        f"Конверсия выполнена успешно!<br>"
        "Источник данных: "
        "<a href='https://cbr.ru/currency_base/daily/' target='_blank'>Центральный банк РФ</a>"
    )


def create_interface():
    with gr.Blocks() as demo:
        # Заголовок и ссылка на источник
        gr.HTML("""
        ...
        # Поле для сообщений со HTML-разметкой (для ссылок)
        error_output = gr.HTML(label="Сообщение")

        def on_convert(amount, from_curr, to_curr):
